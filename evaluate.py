...
from subgradient_logreg_l1 import subgradient_logreg_l1
...
def plot_our_history(X, y, epochs=50, lam=1e-3):
    ...
    w_subgrad, hist_subgrad = subgradient_logreg_l1(X, y, lambd=lam, epochs=epochs, seed=42, learning_rate=0.0001)
    
    plt.plot(hist_plain, label="CD", linestyle="--")
    plt.plot(hist_acc,   label="Accelerated CD")
    plt.plot(hist_fista, label="FISTA", linestyle="--")
    plt.plot(hist_subgrad, label="Subgradient", linestyle="--")
    plt.xlabel("Epoch")
    ...
    y_pred_subgrad = (X @ w_subgrad >= 0).astype(int)
    ...
    print(f"Subgradient: {accuracy_score(y, y_pred_subgrad):.3f}")
    ...
    print(f"Subgradient: {f1_score(y, y_pred_subgrad):.3f}")
